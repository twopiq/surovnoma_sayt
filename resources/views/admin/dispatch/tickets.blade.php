    ...
    <x-slot name="header">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
            ...
            <div class="flex flex-wrap gap-2">
                <a href="{{ route('admin.dispatch.export', array_merge(request()->query(), ['format' => 'excel'])) }}" class="rounded-md border border-slate-300 px-4 py-2 text-sm font-semibold text-slate-700 transition hover:bg-slate-100">
                    Excel eksport
                </a>
                <a href="{{ route('admin.dispatch.export', array_merge(request()->query(), ['format' => 'csv'])) }}" class="rounded-md border border-slate-300 px-4 py-2 text-sm font-semibold text-slate-700 transition hover:bg-slate-100">
                    CSV
                </a>
            </div>
        </div>
