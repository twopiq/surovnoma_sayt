<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
            ...
            <form method="GET" action="{{ route('admin.users.profile') }}" class="flex gap-2">
                <select name="user" class="rounded-md border-slate-300 text-sm shadow-sm">
                    @foreach ($users as $user)
                        <option value="{{ $user->id }}" @selected($selectedUser->id === $user->id)>{{ $user->name }}</option>
                    @endforeach
                </select>
                <button class="rounded-md bg-violet-700 px-4 py-2 text-sm font-semibold text-white">Ko'rish</button>
            </form>
        </div>
    </x-slot>

    <div class="mx-auto grid max-w-7xl gap-6 px-4 pt-8 lg:grid-cols-[0.95fr_1.05fr] sm:px-6 lg:px-8">
        <section class="overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm">
            <div class="h-44 bg-[#123b2f]">
                <div class="h-full w-full bg-[linear-gradient(135deg,#123b2f_0%,#0f8f6b_52%,#fda13f_100%)] opacity-90"></div>
            </div>

            <div class="px-5 pb-6">
                <div class="-mt-14 flex justify-center">
                    <span class="inline-flex h-28 w-28 items-center justify-center rounded-full border-4 border-white bg-emerald-100 text-4xl font-bold text-emerald-700 shadow-sm">
